...
//    private static final int GESTURE_MODE_ONLY_CALENDAR = 1;
    ...
    void updateContentViewTranslateY() {
        ...
        mContentViewTranslateY = mDelegate.getMonthViewShowMode() == CalendarViewDelegate.MODE_ALL_MONTH ? mContentViewTranslateY = 5 * mItemHeight : CalendarUtil.getMonthViewHeight(calendar.getYear(), calendar.getMonth(), mItemHeight, mDelegate.getWeekStart())
                - mItemHeight;
    ...
    final void updateCalendarItemHeight() {
        ...
        mContentViewTranslateY = mDelegate.getMonthViewShowMode() == CalendarViewDelegate.MODE_ALL_MONTH ? 5 * mItemHeight : CalendarUtil.getMonthViewHeight(calendar.getYear(), calendar.getMonth(),
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                 ...
                 * 2、非滚动控件，直接拦截事件
                 */
                boolean equals = mContentView.getTranslationY() == -mContentViewTranslateY;
                boolean values = y >= CalendarUtil.dipToPx(getContext(), 98);
                if (dy > 0 && equals && values) {
                    if (!isScrollTop()) {
                        return false;
                    }
                }
    ...
    final void initStatus() {
        ...
        boolean equals =  mDefaultStatus == STATUS_SHRINK || mCalendarShowMode == CALENDAR_SHOW_MODE_ONLY_WEEK_VIEW;
        if (equals && mCalendarShowMode != CALENDAR_SHOW_MODE_ONLY_MONTH_VIEW) {
            post(new Runnable() {
                @Override
                public void run() {
                    ObjectAnimator objectAnimator = ObjectAnimator.ofFloat(mContentView,
